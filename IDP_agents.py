...
def AgentPytlik(GENtable,varVar,varVal,varMin,varMax):
    ...
    cnnW,crrW = cnt_X('NCC')
    query = """SELECT * FROM """+GENtable+""" where specie like '"""+specie+"""';"""
    crrW.execute(query)
    rows = crrW.fetchall()
    ...
    while iiiii < 432.11:
        ...
        while i < varN:
            ...
            while ii > -0.1:
                specimen[0,i] = specimen[0,i] + (TOPmat[ii,i]-specimen[0,i])*(TOPmat[ii,pheroNo]**2)*0.2
                print(TOPmat[ii,i])
...
def AgentDarwin(specie):
    ...
    cs3min = 75
    cs3max = 250 
    MSmax = 8 #33 is the maximum manderl speed at NCC - could be effectively increased by slowing spool speed
    MSmin = 0.8
    NLmin = 1
    NLmax = 15
    ...
    if is_empty(sd) ==True:
        ...
        while i < population:
            ...
            specimen[i,0] = (random.uniform(0,1))*(cs1max-cs1min)+(cs1min)
            specimen[i,1] = (random.uniform(0,1))*(cs2max-cs2min)+(cs2min)
            specimen[i,2] = (random.uniform(0,1))*(cs3max-cs3min)+(cs3min)
            specimen[i,4] = (random.uniform(0,1))*(MSmax-MSmin)+(MSmin)
            specimen[i,3] = (random.randint(NLmin, NLmax))
            # export the new generation into SQL table
            query = "INSERT INTO ga1(specie,generation,cs1,cs2,cs3, no_layers, mandrel_speed) VALUES("
            query += """'"""+specie+"""','"""+str(1)+"""','"""+str(specimen[i,0])+"""','"""+str(specimen[i,1])+"""','"""+str(specimen[i,2])+"""','"""+str(specimen[i,3])+"""','"""+str(specimen[i,4])+"""')"""
            crrW.execute(query)
            cnnW.commit()
            i= i +1
        maxG = 1
    ...
    for row in rows:
        ...
        fitness,arunID,MS = SingleLoop(float(row[3]),float(row[4]),float(row[5]),float(row[6]),float(row[7]))
        recordID = row[0]
        print("Hurrah! New individual was born into the populaiton!")
        print(row[3],row[4],row[5],row[6],row[7],"stats:",fitness,arunID)
        #stores the result, reference simulaiton number, and mandrel_speed in case change was required due to minimal angle limit
        cnnW,crrW = cnt_X('NCC')
        query = """UPDATE ga1 SET  fitness = """+str(fitness)+""", mandrel_speed = """+str(MS)+""", arunID = """+str(arunID)+""" Where (idnew_table = """+str(recordID)+""");"""
        crrW.execute(query)
        cnnW.commit()
        #close SQL handles 
        dc_X('NCC',cnnW,crrW)     
    ...
    cnnW,crrW = cnt_X('NCC')
    query = """SELECT cs1,cs2,cs3,no_layers,mandrel_speed,fitness FROM ga1 where generation = '"""+str(maxG)+"""' and specie = '"""+specie+"""';"""
    crrW.execute(query)
    pop = np.zeros([1,6])
    popi = np.zeros([1,6])
    rows = crrW.fetchall()
    ...
    while i < mutR:
        ...
        if trait < 3:
            MutVar = (random.uniform(0,1))*(cs3max-cs3min)+(cs3min)
        if trait ==3:
            MutVar = (random.randint(NLmin, NLmax))
        if trait == 4:
            MutVar = (random.uniform(0,1))*(MSmax-MSmin)+(MSmin)
    ...
    while i < newpop.shape[0]:
        query = "INSERT INTO ga1(specie,generation,cs1,cs2,cs3, no_layers, mandrel_speed) VALUES("
        query += """'"""+specie+"""','"""+str(maxG)+"""','"""+str(newpop[i,0])+"""','"""+str(newpop[i,1])+"""','"""+str(newpop[i,2])+"""','"""+str(newpop[i,3])+"""','"""+str(newpop[i,4])+"""')"""
        crrW.execute(query)
        cnnW.commit()
        i = i + 1
    #close SQL handles 
