...
GENtable = "[DIGIProps].[dbo].[_iters_35]"
cnnT,crrT = cnt_X('NCC')
#print(GENtable)
query = """SELECT spools, RAD, mandrel_speed, fitness,arunID FROM """+GENtable+""" where fitness > 0;"""
#print(query)
crrT.execute(query)
rows = crrT.fetchall()
...
for i in rows:
    ...
    Mtamp[0,0] = (i[0])
    Mtamp[0,3] = (i[3])
    query = """SELECT FEfile FROM arun where idArun = """+str(i[4])+""";"""
    crrT.execute(query)
...
print("checkpoint2")
#gamma=0.001 ==> deleted
clf = SVR(kernel='sigmoid', C=100, gamma='auto', epsilon=0.001,coef0=1)
clf.fit(X, y) 
...
#Create first plot
s1 = figure(title="RAD agains fitness",plot_width=500, plot_height=500)
s1.circle( X3[:,1], ver_pred, size=10, color="firebrick", alpha=0.5,legend="predictions")
s1.triangle(X3[:,1],X3[:,4], size=10, color="navy", alpha=0.5,legend="source data")
s1.xaxis.axis_label = 'pos1'
s1.yaxis.axis_label = 'infperc'
#s1.triangle(X3[:,3], x, size=10, color="firebrick", alpha=0.5,legend="predictions")
#Create second plot
s2 = figure(title="spools agains fitness",plot_width=500, plot_height=500)
...
s2.circle(X3[:,0],X3[:,4], color="green", alpha =0.5,legend="source data")
s2.xaxis.axis_label = 'pos2'
s2.yaxis.axis_label = 'inf perc'
s2.legend.location = "top_right"
#Create third plot
s3 = figure(title="mandrel_speed agains fitness",plot_width=500, plot_height=500)
#s2.x_range=Range1d(250, 500)
#s2.y_range=Range1d(80, 105)
#Triangles stand for fully infused datapoints.
s3.triangle(X3[:,2], ver_pred, size=10, color="firebrick", alpha=0.5,legend="predictions")
#Circles stand for failed infusions.
s3.circle(X3[:,2],X3[:,4], color="green", alpha =0.5,legend="source data")
s3.xaxis.axis_label = 'pos2'
s3.yaxis.axis_label = 'inf perc'
