...
def Vf_opt2(alpha,p,Fr):
    ...
    b = Fr*1.5 #this can be adjusted for aleternative elipses
    Area = math.pi*((Fr)**2)
    ...
    x = b*2/math.cos(omega)
    m = b*2*math.tan(omega)
    A11 = m*b*2
    L = x*math.sin(beta)
    N = math.sqrt(abs((L*2)**2-(b*2)**2))
    A12 = N*b*2
    A1 = A12 + A11
    A1t = A1*2
    ...
    #D is the distance taken of yarn through diagonal of open space in A2
    print("p",p,"b",b,"newThick",newThick)
    D = math.sqrt((p-2*b)^2+(newThick)^2)
    #the volume of diagonal fibre / volume of available space
    Vf2 = (D*math.pi*b*newThick)/(4*A2*newThick)
    
    #old version Vf2 = Vf1*0.66 #an estimate of the volume fraction with angled yarn going through thickness
    #this should eventually be improved if this method is to be used
    Vf3 = 0
    Vf = (A3t/Atot)*Vf3+(A2t/Atot)*Vf2+(A1t/Atot)*Vf1
    print("Vf",Vf)
    print("Vf1:",Vf1,"Vf2:",Vf2,"Vf3",Vf3,"A1:",A1t,"A2",A2t,"A3",A3t)
    return(Vf)
